...
def create_bypayment_type_df(df):
    ...
    bypayment_type_df['payment_type'] = pd.Categorical(bypayment_type_df['payment_type'], ["boleto", "credit_card", "debit_card", "not_defined", "voucher"])
    
    return bypayment_type_df

def create_byorder_status_df(df):
    byorder_status_df = df.groupby(by="order_status").customer_id.nunique().reset_index()
    byorder_status_df.rename(columns={
        "customer_id": "customer_count"
    }, inplace=True)
    
    return byorder_status_df
...
all_df = pd.read_csv("d:/submission/dashboard/all_data.csv")
...
sns.barplot(x="payment_installments", y="payment_type", hue="payment_type", data=sum_order_items_df.head(5), palette=colors, ax=ax[0], legend=False)
...
sns.barplot(x="payment_installments", y="payment_type", data=sum_order_items_df.sort_values(by="payment_installments", ascending=True).head(5), palette=colors, ax=ax[1])
...
with col2:
    ...
    sns.barplot(
        y="customer_count", 
        x="payment_type",
        data=bypayment_type_df.sort_values(by="payment_type", ascending=False),
        palette=colors,
        ax=ax
    )
...
sns.barplot(
    x="customer_count", 
    y="order_status",
    data=byorder_status_df.sort_values(by="customer_count", ascending=False),
    palette=colors,
    ax=ax
)
